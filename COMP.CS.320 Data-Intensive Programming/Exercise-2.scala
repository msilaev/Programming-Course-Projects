...
val weatherDataFrame = "abfss://shared@tunics320f2023gen2.dfs.core.windows.net/exercises/ex2/rdu-weather-history.csv"
...
val weatherSample: Array[Row] = df_csv.take(5);
...
val aux: Double =df_csv.select(min("temperaturemin")).take(1)(0).getAs[Double](0)
println(aux.getClass.getSimpleName)

// COMMAND ----------

val maxTemp: Double = df_csv.select(max("temperaturemax")).take(1)(0).getAs[Double](0)
val minTemp: Double  =df_csv.select(min("temperaturemin")).take(1)(0).getAs[Double](0)
...
val weatherDataFrameWithYear: DataFrame = df_csv.withColumn("year", year(col("date")))
weatherDataFrameWithYear.printSchema()

// COMMAND ----------

// MAGIC %md
// MAGIC Same using SQL below:

// COMMAND ----------

 df_csv.createOrReplaceTempView("df_view")
val weatherDataFrameWithYear_sql: DataFrame = spark.sql(""" select *,
year(date) as year from df_view 
""")

weatherDataFrameWithYear_sql.printSchema()
...
val aggregatedDF: DataFrame = weatherDataFrameWithYear.groupBy("year").agg(max("temperaturemax").alias("max_temperature"), min("temperaturemin").alias("min_temperature")).orderBy(desc("year"))
aggregatedDF.show()
...
// COMMAND ----------
 ...
 weatherDataFrameWithYear.createOrReplaceTempView("weatherDataFrameWithYear_view")

val aggregatedDF_sql: DataFrame = spark.sql(""" select year, max(temperaturemax) as max_temperature,  
min(temperaturemin) as min_temperature from weatherDataFrameWithYear_view  group by year order by year desc 
""")

aggregatedDF_sql.show()
...
val task6DF: DataFrame =  weatherDataFrameWithYear.groupBy("year").agg(max("temperaturemax").alias("max_temperature"), min("temperaturemin").alias("min_temperature"), count("year").alias("entries"), avg("avgwindspeed").alias("avg_windspeed") ).orderBy(asc("year")).withColumn("avg_windspeed", round(col("avg_windspeed"), 2))
...
weatherDataFrameWithYear.createOrReplaceTempView("weatherDataFrameWithYear_view")
val task6DF_sql: DataFrame =  spark.sql("""  select max(temperaturemax) as max_temperature, min(temperaturemin) as min_temperature, count(year) as entries, round(avg(avgwindspeed), 2) as avg_windspeed from weatherDataFrameWithYear_view group by year order by year asc
 """)
task6DF_sql.show()
...
val min2012: Double =  task6DF.filter(col("year") === 2012).select("min_temperature").take(1)(0).getDouble(0)
val max2016: Double =  task6DF.filter(col("year") === 2016).select("max_temperature").take(1)(0).getDouble(0)
val entries2018: Long = task6DF.filter(col("year") === 2018).select("entries").take(1)(0).getLong(0)
val wind2008: Double = task6DF.filter(col("year") === 2008).select("avg_windspeed").take(1)(0).getDouble(0)
...
task6DF.createOrReplaceTempView("task6DF_view")

val min2012_sql: Double = spark.sql(""" select  min_temperature from task6DF_view where year =2012 
""").take(1)(0).getDouble(0)

val max2016_sql: Double = spark.sql(""" select max_temperature from task6DF_view where year = 2016""").take(1)(0).getDouble(0)

val entries2018_sql: Long = spark.sql(""" select entries from task6DF_view where year = 2018 """).take(1)(0).getLong(0) 

val wind2008_sql: Double = spark.sql(""" select avg_windspeed from task6DF_view where year = 2008 """).take(1)(0).getDouble(0)
...
val min2012 =  weatherDataFrameWithYear.select("year", "fog").filter(col("fog") === "Yes").groupBy("year").agg(count("year")).orderBy(desc("count(year)"))
//filter(col("fog") === "Yes")
val year: Integer = min2012.take(1)(0).getInt(0)
println(year)

// COMMAND ----------

val yearWithMostDaysWithFog: Int = weatherDataFrameWithYear.select("year", "fog").filter(col("fog") === "Yes").groupBy("year").agg(count("year")).orderBy(desc("count(year)")).take(1)(0).getInt(0)
...
val yearWithMostDaysWithFog: Int = spark.sql(""" select year, count(year) as count from weatherDataFrameWithYear_view where fog = 'Yes'  group by year  order by count desc 
""").take(1)(0).getInt(0)

println(yearWithMostDaysWithFog)
